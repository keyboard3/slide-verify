...
export default class SlideVerify {
  ...
  constructor(
    ...
    let canvasWrapper = childNodes[0] as HTMLElement;
    let canvas = canvasWrapper.childNodes[0] as HTMLCanvasElement;
    let loadingIcon = canvasWrapper.childNodes[1] as HTMLElement;
  ...
  initImg() {
    ...
    const img = createImg(() => {
      ...
      if (this.source) {
        ...
        (this.canvasCtx.drawImage as any)(
          img,
          ...this.source,
          0,
          0,
  ...
  clean(): void {
    const { svCanvasWidth, svCanvasHeight } = config;
    (this.canvasCtx as CanvasRenderingContext2D).clearRect(
      0,
      0,
      svCanvasWidth,
      svCanvasHeight,
    );
    (this.blockCtx as CanvasRenderingContext2D).clearRect(
      0,
      0,
      svCanvasWidth,
      svCanvasHeight,
  ...
  bindEvents() {
    ...
    const handleDragEnd = (e: MouseEvent | TouchEvent) => {
      ...
      if (spliced) {
        if (verified) {
          (<HTMLElement> this.sliderIcon.childNodes[0]).innerHTML =
  ...
  reset() {
    ...
    (<HTMLElement> this.sliderIcon.childNodes[0]).innerHTML =
      `<i class="fas fa-bars fa-rotate-90" aria-hidden="true"></i>`;
